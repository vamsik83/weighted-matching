...
void ComputeInitialExtremeMatch(ValueType *u,ValueType *v,ValueType *clabel,ValueType *C,
			...
			for(k=rowptr[i];k<rowptr[i+1] && !(m_inv[i]);k++){
				...
				if(fabs(C1k-0.0) <= 1.0e-30){
					...
					for(k1=rowptr[i1];k1<rowptr[i1+1];k1++){
						...
						if(fabs(C1k-0.0) <= 1.0e-30 && !(m[colind[k1]])){
							/*augment the match.*/
							m[colind[k]] = i;
							m_inv[i] = colind[k];
							clabel[i] = C[k];
...
template <class RowStarts,class ColIndices,class ValueType,class Indices>
void ANSTMC64(RowStarts *rowptr,ColIndices *colind,ValueType *val,Indices n,Indices *perm){
	ValueType *u,*v,*dist;
	Indices *p,*m_inv;
	CreateWorkSpace<double,node_t>(&u,&v,&m_inv,&p,&dist,n);
	LogTransformT(rowptr,colind,val,n);
...
ValueType *dist,ValueType *u,ValueType *v,Indices *p,
Indices *m_inv,Indices *m, Indices n,CompareFunction cmpFunc){
	...
	ValueType *clabel = new ValueType[n];
	ValueType *aug_label = new ValueType[n];
...
void LogTransformT(RowStarts *rowptr,ColIndices *colind,ValueType *val,Indices n){
	ValueType *max_c = new ValueType[n];
	rowptr--;colind--;val--;max_c--;
	Indices nnz_size = (Indices)0; 
	Indices k,i,j;
	Clock *anst_clk;
	for(j=1;j<=n;j++){
...
int main(int argc,char** argv){
	...
	LogTransformT(G->rowptr,G->colind,G->nnz,G->order);
	WeightedMatching<node_t,node_t,double,KeyCompare>(G->rowptr,G->colind,G->nnz,dist,
		u,v,p,m_inv,m->m,G->order,DoubleCompare);
#else

#ifndef USE_MC21 
	ANSTMC64<node_t,node_t,double,node_t>(G->rowptr,G->colind,G->nnz,G->order,(m->m));
#else
	ANSTMC21<node_t,node_t,node_t>(G->rowptr,G->colind,G->order,(m->m));
#endif /*USE_MC21*/
	...
	printf("Ticks(Initial Match) = (%d) , Ticks(Core) = (%d),LogTransform(%d)\n",(int)InitialMatchTicks,
		(int)WeightedMatchTicks,(int)anst_ticks);

 ...
 *
 * 09/16/2008 Begin updating for getting
 * performance numbers for the dual update
 * technique.
 *
 */
#define BINARY_HEAP 1

#if 0 /*Turn this ON when you turnoff BINARY_HEAP, WARNING: ONLY for DEV purposes*/
#define TURN_ON_SAVE_HEAP 1
#define USE_BIT_ARRAY 1
#endif 

/*Build with FAST_PRE_ORDER to get 
 *an executable with our new idea.*/
#ifdef FAST_PRE_ORDER
#define FAST_DUAL_UPDATE 1
#define NO_1_STEP_AUG 1
#endif
...
#include "Heap.h"
#include "Bipartite.h"
#define MAX_DOUBLE_VALUE1 1.7976931348622e+308
#define MAX_DOUBLE_VALUE HUGE_VAL
int anst_ticks;
...
#ifndef NO_1_STEP_AUG
	/*Update Cost and match.*/
	for(i=1;i<=n;i++){
		for(k=rowptr[i];k<rowptr[i+1];k++){
			C1k = C[k]-v[i]-u[colind[k]]; /*to handle -0.0*/
			if(fabs(C1k-0.0) <= 1.0e-30 && (!m[colind[k]] && !m_inv[i])){
				m[colind[k]] = i;
				m_inv[i] = colind[k];
				clabel[i] = k;
			}
			...
			for(k=rowptr[i];k<rowptr[i+1] && !(m_inv[i]);k++){
				...
				if(C1k <= 1.0e-30){
					...
					for(k1=rowptr[i1];k1<rowptr[i1+1];k1++){
						C1k = fabs(C[k1] - v[i1]-u[colind[k1]]);
						if(C1k <= 1.0e-30 && !(m[colind[k1]])){
							/*augment the match.*/
							m[colind[k]] = i;
							m_inv[i] = colind[k];
							clabel[i] = k;
...
/*******Dual Update***********/
			...
			for(j=1;j<=n;j++){
#ifndef USE_BIT_ARRAY
				if(col_marker[j] == (Indices)i){
#else
				if(CheckBit(col_marker,j)){
#endif
					u[j] = (u[j]+dist[j])-curr_aug_path;
...
ValueType *v,ValueType *dist,Indices *m,Indices *m_inv){
	...
	for(i=1;i<=n;i++){
		for(k=rowptr[i];k<rowptr[i+1];k++){
			...
			/*if(val[k]<=1.0e-100){
				val[k] = HUGE_VAL;
				//val[k] = (double) MAX_DOUBLE_VALUE1/n;
			}else{*/
				if(max_c[colind[k]]==0.0/*1.0e-100*/){
					val[k] = HUGE_VAL;
					//val[k] = (double) MAX_DOUBLE_VALUE1/n;
					//val[k] = MAX_DOUBLE_VALUE1/n - log10(val[k]);
				}else{
...
int main(int argc,char** argv){
	...
	if(argc >= 3){
		perm_file_output = argv[2];
		fprintf(stdout,"The permutation will be printed into %s \n",perm_file_output);
	}
	...
	if(argc > 3 && argv[3]){ 
		/*Initial Match File Can be Passed here*/
...
/*Verify if we have Intel MKL library, other wise
 *don't verify
 */

#ifdef VERIFY_INTEL_MKL 
...
/*Lets call this first*/
	...
	if(failed){
		if(CompareDoubles(unmatched_sum_i1,unmatched_sum_i2)){
...
#else /*VERIFY_INTEL_MKL*/
/*Write the permutation into .perm file if optional */
	FILE *perm_output = NULL;
	if(perm_file_output){
		perm_output = fopen(perm_file_output,"w");
		if(!perm_output){
			fprintf(stdout,"WARNING: Ignoring the output \n");
		}else{
			for(i=0;i<G->order;i++){
				fprintf(perm_output,"%u ",(m->m)[i]);
			}
			fclose(perm_output);
		}
	}
#endif /*VERIFY_INTEL_MKL*/

...
inline void Heapify(Heap *h,unsigned int i){
	...
	while(i<=(n/2)){
		index = ((2*i+1)>n)?(2*i):((h->cmp((void *)harray[2*i],
			(void *)harray[2*i+1]))? (2*i):(2*i+1));
#ifndef NO_ASSERTS 
		assert(i< n && index <=n);
#endif
		if(!((h->cmp)(harray[i],harray[index]))){ 
			/*Update the indices to help decrease key.*/
			if(h->keyMap && h->GetKeyID){
				kid1 = (h->GetKeyID)(harray[i]);
				kid2 = (h->GetKeyID)(harray[index]);
#ifndef NO_ASSERTS
...
unsigned int InsertHeap(Heap *h,void *key){
	...
	while(i>1){
		if((h->cmp)(harray[i],harray[i/2])){
			if(h->keyMap && h->GetKeyID){
				/*Supporting Decrease Key Operation*/
				kid1 = (h->GetKeyID)(harray[i]);
				kid2 = (h->GetKeyID)(harray[i/2]);
#ifndef NO_ASSERTS
...
Heap* NewBinaryHeap(KeyCompare cmp,unsigned int keyMapSize,
	...
	h->n = 0;
	h->heapArray = (unsigned long **)malloc(sizeof(unsigned long *)*(HEAP_START_SIZE));
	h->fill = HEAP_START_SIZE;
	assert(h->heapArray);
	h->cmp = cmp;
	(h->heapArray)--; 
	return h;
...
unsigned int DecreaseKey(Heap *h,keyid_t kid){
	...
	while(i>1){
		if((h->cmp)(harray[i],harray[i/2])){
			...
			kid1 = (h->GetKeyID)(harray[i/2]);
			kid2 = (h->GetKeyID)(harray[i]);
#ifndef NO_ASSERTS
...
void* GetMin(Heap *h){
	return ((unsigned long **)h->heapArray)[1];
}
